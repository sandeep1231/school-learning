...
/**
 * Hybrid retrieval: pgvector cosine + Postgres FTS, scoped to a topic and
 * optionally its neighbours. Returns top-k ranked chunks.
 *
 * DB helper functions `match_chunks_vector` and `match_chunks_fts` are
 * expected to exist (see supabase/migrations/0003_rag_functions.sql).
 *
 * @deprecated Prefer `retrieveForScope({ topicId, ... })`. Kept as an
 *             adapter for one release; remove after all callers migrate.
 */
export async function retrieveForTopic(opts: {
  query: string;
  topicId: string;
  includeNeighbours?: boolean;
  k?: number;
  ...
  return retrieveForScope({
    query: opts.query,
    topicId: opts.topicId,
    includeNeighbours: opts.includeNeighbours,
    k: opts.k,
...
/**
 * Subject-scoped hybrid retrieval. Used as a fallback when chunks aren't
 * topic-tagged (bulk-ingested textbook PDFs) and for the guest path where
 * we only know the curriculum subject code.
 *
 * @deprecated Prefer `retrieveForScope({ subjectCode, ... })`. Kept as an
 *             adapter for one release; remove after all callers migrate.
 */
export async function retrieveForSubject(opts: {
  query: string;
  subjectCode: string;
  k?: number;
  language?: AppLanguage;
